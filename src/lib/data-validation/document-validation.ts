...
export const validateDocument = (documentData: DocumentInput): ValidationResult => {
  ...
  if (
    !documentData.documentAccess ||
    (documentData.documentAccess !== "EXCLUDE_SUBJECT_TEACHERS" && documentData.documentAccess !== "ALL_WITH_STUDENT_ACCESS" && documentData.documentAccess !== "ONLY_CREATOR")
  ) {
    return { valid: false, message: "Document access is required and must be either 'EXCLUDE_SUBJECT_TEACHERS', 'ALL_WITH_STUDENT_ACCESS', or 'ONLY_CREATOR'." }
  }
