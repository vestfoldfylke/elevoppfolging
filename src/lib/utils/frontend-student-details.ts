...
export const getFrontendStudentDetails = (student: FrontendStudent | FrontendOverviewStudent, APP_INFO: ApplicationInfo): FrontendStudentDetails => {
  ...
  const mainClassMembership = student.mainEnrollment?.classMemberships
  ...
  const mainContactTeacherGroupMembership = student.mainEnrollment?.contactTeacherGroupMemberships
    .map((membership) => ({ ...membership, period: getPeriodDetails(membership.period, APP_INFO) }))
    .find((membership) => membership.period.active || membership.period.withinViewAccessWindow)
