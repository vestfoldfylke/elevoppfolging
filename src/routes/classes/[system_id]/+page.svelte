<script lang="ts">
  ...
  import { canEditGroupDocument, isSchoolLeaderForSchool } from "$lib/shared-authorization/authorization"
  ...
  <div class="documents">
    ...
    {:else}
      ...
      {#each filteredDocuments as document (document._id)}
        <DocumentComponent {document} accessSchools={[data.classGroup.school]} canEditDocument={canEditGroupDocument(data.authenticatedPrincipal, document)} canRemoveDocument={isSchoolLeaderForSchool(data.principalAccess, document.school.schoolNumber)} groupName={data.classGroup.name} />
      {/each}
    {/if}
  </div>
</div>
