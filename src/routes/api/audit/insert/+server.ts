...
import { validateAuditEntryInput } from "$lib/data-validation/audit-entry-validation"
...
const addAuditEntry: ApiNextFunction<AuditEntryInputResponse, AuditEntryInputRequest> = async ({ body }) => {
  ...
  if (!body.errorMessage || !body.errorMessageObject) {
    throw new HTTPError(400, "'errorMessage' and 'errorMessageObject' are required in request body")
  }

  const validationResult: ValidationResult = validateAuditEntryInput(auditEntry)
  if (!validationResult.valid) {
    throw new HTTPError(400, `Invalid audit entry: ${validationResult.message}`)
  }
